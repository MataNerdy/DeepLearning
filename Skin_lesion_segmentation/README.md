...
Tversky Loss показал лучший результат для медицинской сегментации благодаря асимметричному штрафу за false negatives и false positives. Для задач lesion segmentation это особенно важно, поскольку пропуск части поражения обычно критичнее небольшого переоценивания маски.
...
- Архитектура U-Net существенно лучше подходит для медицинской сегментации малых датасетов благодаря skip-connections.
- Tversky Loss оказался наиболее устойчивым для lesion segmentation.
- BCE Loss недостаточно хорошо оптимизирует форму маски и хуже работает для несбалансированной сегментации.
- SegNet хуже сохраняет пространственную информацию и склонен к шумовым предсказаниям.
- Даже на небольшом датасете PH2 U-Net достиг Dice ≈ 0.90 без pretrained encoder.
